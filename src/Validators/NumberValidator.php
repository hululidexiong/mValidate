...
class NumberValidator extends Validator
{
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            $this->message = $this->integerOnly ? Bear::t( '{attribute} must be an integer.')
                : Bear::t( '{attribute} must be a number.');
        }
        if ($this->min !== null && $this->tooSmall === null) {
            $this->tooSmall = Bear::t( '{attribute} must be no less than {min}.');
        }
        if ($this->max !== null && $this->tooBig === null) {
            $this->tooBig = Bear::t( '{attribute} must be no greater than {max}.');
        }
    }
    ...
    public function validateAttribute($model, $attribute)
    {
        ...
        if (!preg_match($pattern, BaseStringHelper::normalizeNumber($value))) {
            $this->addError($model, $attribute, $this->message);
        }
    ...
    protected function validateValue($value)
    {
        if (is_array($value) || is_object($value)) {
            return [ Bear::t( '{attribute} is invalid.'), []];
        }
        $pattern = $this->integerOnly ? $this->integerPattern : $this->numberPattern;
        if (!preg_match($pattern, BaseStringHelper::normalizeNumber($value))) {
            return [$this->message, []];
